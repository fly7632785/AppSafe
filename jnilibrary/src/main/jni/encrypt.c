...
Java_com_jafir_jnilibrary_Hello_staticSayHello(JNIEnv *env, jclass hello) {
    //调用java的static
    jmethodID function = (*env)->GetStaticMethodID(env, hello,"staticFunction","(ILjava/util/Date;[I)I");
...
Java_com_jafir_jnilibrary_Hello_callArray(JNIEnv *env, jobject instance) {
    jfieldID arraysF = (*env)->GetFieldID(env,(*env)->GetObjectClass(env,instance),"arryas","[I");
    jfieldID personsF = (*env)->GetFieldID(env,(*env)->GetObjectClass(env,instance),"persons","[com/jafir/jnilibrary/Person");
    jintArray arrays = (*env)->GetObjectField(env,instance,arraysF);
    //获取数组对象指针
    jint *int_arr = (*env)->GetIntArrayElements(env,arrays,NULL);
    for (int i = 0; i < (*env)->GetArrayLength(env, int_arr); ++i) {
        LOGD("1");
    }
}